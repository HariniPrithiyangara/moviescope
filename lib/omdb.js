...
const searchMockMovies = (query, page = 1) => {
  ...
  const limit = 12; // R1: exactly 12 results per page
...
// R1: returns exactly 12 movies per page
const ITEMS_PER_PAGE = 12;
const OMDB_PAGE_SIZE = 10;

export async function searchMovies(query, ourPage = 1) {
  const trimmedQuery = query.trim() || 'Star';
  const cacheKey = `search_${trimmedQuery.toLowerCase()}_page${ourPage}`;
  ...
  try {
    // Calculate which OMDb pages we need to cover our 12-item page
    const startIdx = (ourPage - 1) * ITEMS_PER_PAGE;        // 0-indexed first item we need
    const endIdx = ourPage * ITEMS_PER_PAGE - 1;             // 0-indexed last item we need
    const omdbPageStart = Math.floor(startIdx / OMDB_PAGE_SIZE) + 1;
    const omdbPageEnd   = Math.floor(endIdx   / OMDB_PAGE_SIZE) + 1;

    // Fetch all required OMDb pages in parallel
    const pagePromises = [];
    for (let p = omdbPageStart; p <= omdbPageEnd; p++) {
      pagePromises.push(
        fetch(`${BASE_URL}?s=${encodeURIComponent(trimmedQuery)}&page=${p}&apikey=${API_KEY}`)
          .then((r) => r.json())
      );
    }
    const omdbPages = await Promise.all(pagePromises);

    // Validate first page
    if (!omdbPages[0] || omdbPages[0].Response !== 'True') {
      throw new Error(omdbPages[0]?.Error || 'No results found.');
    }

    const totalResults = parseInt(omdbPages[0].totalResults, 10);

    // Combine all fetched OMDb search items into one array
    let allItems = [];
    for (const pg of omdbPages) {
      if (pg.Response === 'True' && pg.Search) {
        allItems = allItems.concat(pg.Search);
      }
    }

    // Slice exactly 12 items relative to startIdx within the fetched block
    const blockOffset = (omdbPageStart - 1) * OMDB_PAGE_SIZE;
    const sliceStart = startIdx - blockOffset;
    const pageItems = allItems.slice(sliceStart, sliceStart + ITEMS_PER_PAGE);

    // Fetch full details for each item in parallel (uses cache layer to avoid duplicates)
    ...
    // 3. Fallback: Search mock database (also uses 12 per page)
    return searchMockMovies(trimmedQuery, ourPage);
  }
}
